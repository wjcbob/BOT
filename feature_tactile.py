...
def main(start, end):
    t0 = time.time()
    acc_list, loss_list = np.array([]), np.array([])
    for epoch in range(start, end):
        model_name = path+'/'+str(epoch)+'_model.pkl' 
        model.load_state_dict(torch.load(model_name))
        test_loss, test_acc = deploy(epoch, DEVICE)
        acc_list = np.append(acc_list, test_acc)
        loss_list = np.append(loss_list, test_loss.cpu().numpy())
        t1 = time.time()
        print(t1-t0)
    print(acc_list)
    print(loss_list)
    sio.savemat(path+'/pred_test.mat', {'acc':acc_list, 'loss':loss_list}) 

def show():
    data = sio.loadmat(path+'/pred_test.mat')
    acc_list = data['acc']
    loss_list = data['loss']
    print(acc_list)
    print(loss_list)
...
def feature(epoch, device):
    ...
    data_train = data_train.reshape(-1,96)
    data_test = data_test.reshape(-1,96)
    label_train = label_train.reshape(-1,1)
    label_test = label_test.reshape(-1,1)
    print(data_train.shape,data_test.shape)
    print(label_train.shape,label_test.shape)
    
    sio.savemat('./data/tactile_feature_train.mat', 
                {'data':data_train, 'label':label_train})
    sio.savemat('./data/tactile_feature_test.mat', 
